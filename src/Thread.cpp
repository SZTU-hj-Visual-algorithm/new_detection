...
//				lin_is_get = port.get_Mode1_new(mode_temp, lin[0], lin[1], lin[2], quat);    // uart
                ...
                pthread_mutex_lock(&mutex_new);
                {
                    ...
                    _send_data = {serial.vision_msg_.id,
                                    serial.vision_msg_.mode,first_get, //! 0x21应该换成serial.vision_msg_.mode赋值，方便调试默认给的0x21
...
void* Implement(void* PARAM)
{
    ...
    while (is_continue)
    {
        ...
        id_temp = _send_data.id;
        mode_temp = _send_data.mode;
        second_get = _send_data.dat_is_get;
        time_temp = _send_data.tim;
        speed = _send_data.data[2];
        pthread_mutex_unlock(&mutex_new);
        // TODO: 454558845848545

//        printf("[mode_temp]:    |%x\n",mode_temp);
//        printf("[id_temp]:    |%d\n",id_temp);
//        mode_temp = 0x21;
        if(second_get && (mode_temp == 0x21 || mode_temp == 0x11))
        {
//            printf("[mode_temp]:    |%x\n",mode_temp);
//            printf("[bullet_speed]:    |%lf\n",speed);
//            printf("[id_temp]:    |%d\n",id_temp);
            int color;
            if(id_temp == 7)
            {
            ...
            Targets = Detect.autoAim(src_copy, color);


//            cv::circle(src,cv::Point(640,512),20,cv::Scalar(255,255,255),-1);

            if(speed < 13)
            {
                Track.AS.bullet_speed = 13.5;
            }
            else
            {
                Track.AS.bullet_speed = speed;
            }

//            Track.AS.bullet_speed = 15;

            if (Track.locateEnemy(src_copy,Targets,time_temp, mode_temp))
            {
